...
def all_activities_stats(write = False, unix_start = datetime(2023,1,1).timestamp()): 
    if write:
        ...
        pd.read_csv("test activities22.csv")
...
def get_athlete_tags(write:bool,after_timestamp=datetime(2023,1,1).timestamp()):
    ...
    merged = pd.merge(pd.read_csv("athlete_info.csv"),pd.read_csv('id_date_match.csv'),left_on='practice_tag',right_on='ids')
    merged.to_csv("date_athlete_tag_merged.csv")
    data = pd.read_csv(variables.data_path)
    print()
    
    athlete_info_df.to_csv('athlete_info.csv',index=False)
...
def merge_info_date_data(raw_data = 'activities22.csv', out_path = "tagged_no_ac_data.csv" ):
    ...
    #pd.merge(pd.read_csv("athlete_info.csv"),pd.read_csv('id_date_match.csv'),left_on='practice_tag',right_on='ids').to_csv("date_athlete_tag_merged.csv")
    merged = pd.merge(pd.read_csv("athlete_info.csv"),pd.read_csv('id_date_match.csv'),left_on='practice_tag',right_on='ids')
...
def activitity_ids_2(after_date = 1641026743, write = True):
    ...
    df = df.groupby("date_id")['tag'].apply(tag_consolidate)
    df.to_csv('2022 tags_2.csv')
    
    return [df, ids]

'''
data = pd.read_csv("test_big_merge_2.csv")
