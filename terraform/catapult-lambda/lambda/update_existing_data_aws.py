...
def activitity_ids_2(after_date=1641026743, write=True):
    ...
    #df["ids"] = ids_df["ids"]
    #df.to_csv('2022 tags_2.csv') #removing this for now as i dont think tags 2022_2 are needed to be read again, we can just save them and reuse variable 
...
def get_new_stats(new_ids):
    ...
    for practice_id in new_ids:
        ...
        #merged.to_csv("date_athlete_tag_merged.csv")
    
    dfs.to_csv("activities22.csv")
    athlete_info_df.to_csv("athlete_info.csv")
...
def update_existing_data_aws():
    ...
    # %%
    #origingal_id_date_match = pd.read_csv('id_date_match.csv')
    ...
    new_athlete_info_and_tags.to_csv('new_athlete_info_and_tags.csv',index=False)
    #quit()
    # %%
    ...
    updated_activities.to_csv('activities22.csv',index = False)
    ...
    updated_tagged_no_ac.to_csv('tagged_no_ac.csv')
    ...
    updated_ac_data['tag']
    updated_ac_data[updated_ac_data['athlete_name'] == 'Sam Jackson']
    updated_ac_data
    ...
    # %%
    #updated_position_summary.to_csv('position_summary.csv')
    #updated_team_summary.to_csv('team_summary_2.csv')
    ...
    # %%
    updated_team_summary.to_csv('team_summary_2.csv',index = False)
    updated_position_summary.to_csv('position_summary.csv',index = False)
    updated_ac_data.to_csv('full_ac_data.csv',index = False)
    ...
    recent_ac_data.to_csv('recent_ac_data.csv',index = False)
